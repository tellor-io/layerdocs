...
1\) Change directory to `~/layer/binaries/v6.1.6` and check if your address has funds. You will need to have at least 1 TRB to stake a validator on tellor layer:

{% code overflow="wrap" %}
```bash
cd ~/layer/binaries/v6.1.6 && ./layerd query bank balance YOUR_ACCOUNT_NAME loya
```
{% endcode %}
